...
public class MovieAllBoard extends JFrame {
	...
	public MovieAllBoard() {
		...
		/*게시글 넣는 부분*/
		PagingDTO Pdto = new PagingDTO(dtos.size(), 1);
		...
		/*페이지 번호 버튼 넣는 반복문*/
		insertPageNum(contentPanel, pagingPanel, );
	...
	private void insertPageNum(JPanel contentPanel, JPanel pagingPanel) {
		for(int i=0;i<10;i++) {
...
//					System.out.println(pageBtn.getText()); //각 숫자버튼 구분용.
					...
					PagingDTO Pdto = new PagingDTO(dtos.size(), Integer.parseInt(pageBtn.getText()));
					inputTopPanel(contentPanel);
					insertBoardElement(contentPanel,Pdto);
	...
	private void inputTopPanel(JPanel contentPanel) {
	...
	private void insertBoardElement(JPanel contentPanel, PagingDTO Pdto) {
		for(int i=Pdto.getWriting_Start();i<=Pdto.getWriting_End();i++) {
